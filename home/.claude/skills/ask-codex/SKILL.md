...
```bash
codex -a never -s read-only exec "Your question or task here" < /dev/null
```

> `< /dev/null` は必須。理由は下記 Notes を参照。
...
- 実行に時間がかかる可能性があるためBashツールのタイムアウトを10分に延長する
- **stdinは必ず `< /dev/null` で閉じる**: `codex exec` は stdin が TTY でないと、引数で渡したプロンプトに加えて stdin からの追加入力を EOF まで読み込もうとする (`Reading additional input from stdin...` のまま停止する)。Bash ツール (特に `run_in_background: true`) からの実行は非TTYのためハングする。`echo "..." | codex exec -` のように stdin で渡す場合は不要。
